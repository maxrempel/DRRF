...
if __name__ == 'purine_homol_T90_1_same':
            ...
            with ThreadPoolExecutor() as executor:
                for row in chimeric_reads.itertuples(index=False):
                    for record in records:
                        sequence = record.seq
                        executor.submit(homologs_blast, row, sequence, str("same"), pbar)
        if os.path.exists(f"{folder_chr}/dotplots_same/LigP_finder_output_same.tsv"):
            df = pd.read_csv(f"{folder_chr}/dotplots_same/LigP_finder_output_same.tsv", sep='\t', header=None)
            ...
            df.to_csv(f"{folder_chr}/dotplots_same/LigP_finder_output_same.tsv", header=True, index=False, sep='\t')
