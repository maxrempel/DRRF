...
from __main__ import homologs_blast

if __name__ == 'purine_homol_T90_1_opposite':
    def process_opp(folder_chr, chromosome):
        ...
        chimeric_reads.columns = ['read_id', 'continuous_read', 'flag1', 'internal_arm', 'internal_cigar', 'flag2', 'terminal_arm','terminal_cigar']
        ...
        with tqdm(total=len(chimeric_reads)) as pbar:
            with open(f"{folder_chr}/{chromosome}.fa", "r") as handle:
                records = list(SeqIO.parse(handle, "fasta"))
            with ThreadPoolExecutor() as executor:
                for row in chimeric_reads.itertuples(index=False):
                    for record in records:
                        sequence = record.seq
                        executor.submit(homologs_blast, row, sequence, str("opposite"), pbar)
        if os.path.exists(f"{folder_chr}/dotplots_opposite/LigP_finder_output_opposite.tsv"):
            df = pd.read_csv(f"{folder_chr}/dotplots_opposite/LigP_finder_output_opposite.tsv", sep='\t', header=None)
            ...
            df.to_csv(f"{folder_chr}/dotplots_opposite/LigP_finder_output_opposite.tsv", header=True, index=False, sep='\t')
